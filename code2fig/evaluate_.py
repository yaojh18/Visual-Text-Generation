...
def calculate_bleu_score(text1, text2):
    return bleu.compute(predictions=[text1], references=[[text2]])['bleu']


def calculate_bert_score(text1, text2):
    return bert_score.compute(predictions=[text1], references=[text2], lang='en')['f1'][0]
...
if __name__ == "__main__":
    ...
    res_pd = pd.DataFrame(columns=['category', 'edit_distance', 'bleu_score', 'bert_score'])
    for cat in cat_list:
        sub_cat_list = os.listdir(os.path.join(output_dir, cat))
        for sub_cat in sub_cat_list:
            ...
            image_id_list = os.listdir(os.path.join(output_dir, cat, sub_cat))
            for image_id in image_id_list:
                circle_id_list = os.listdir(os.path.join(output_dir, cat, sub_cat, image_id))
                for circle_id in circle_id_list:
                    prediction = ''.join(open(os.path.join(output_dir, cat, sub_cat, image_id, circle_id), encoding='utf-8').readlines())
                    ...
                    bert_score += calculate_bert_score(prediction, ground_truth)
                    count += 1
            res_pd.loc[len(res_pd)] = [full_cat_name, edit_distance / count, bleu_score / count, bert_score / count]
